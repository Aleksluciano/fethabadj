...
module.exports = () => {
	...
	app.patch("/", async(req, res) => {
		...
		const query_edit_vigencia =
			`UPDATE "fethabadj.fethabadj_db.data::VIGENCIA" SET DT_FIN = ?, REGRA = ?
				where EMPRESA = ? AND FILIAL = ? AND DT_INI = ?`;
		try {
			let vigencia_edited = await client.exec(query_edit_vigencia, [period2f, regra, company, branch, period1f]);
			let msg = '';
