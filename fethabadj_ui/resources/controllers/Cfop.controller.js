sap.ui.define([
	...
	return BaseController.extend("fethabadj_ui.controllers.Cfop", {
		onInit: function () {

			var xhttp = new XMLHttpRequest();
			var view = this;

			xhttp.open("GET", "/fethab/cfop/alldata", true);
			xhttp.send();
		...
		save: function () {
			...
			let newdate1 = new Date(oData2.SelectedPeriod.slice(6), oData2.SelectedPeriod.slice(3, 5) - 1, oData2.SelectedPeriod.slice(0, 2),0,0,0);
			let newdate2 = new Date(oData2.SelectedPeriod2.slice(6), oData2.SelectedPeriod2.slice(3, 5) - 1, oData2.SelectedPeriod2.slice(0, 2),0,0,0);
			...
			var xhttp = new XMLHttpRequest();
			xhttp.open("POST", "/fethab/cfop/add", true);
		...
		save2: function () {
			...
			let newdate1 = new Date(data.SelectedPeriod.slice(6), data.SelectedPeriod.slice(3, 5) - 1, data.SelectedPeriod.slice(0, 2),-3,0,0);
			let newdate2 = new Date(data.SelectedPeriod2.slice(6), data.SelectedPeriod2.slice(3, 5) - 1, data.SelectedPeriod2.slice(0, 2),-3,0,0);
			if (newdate1.getTime() > newdate2.getTime()) {
				MessageBox.warning("Período inicial maior que final");
				return;
			}
			...
			var xhttp = new XMLHttpRequest();
			xhttp.open("PATCH", "/fethab/cfop/edit", true);
			xhttp.setRequestHeader('content-type', 'application/json');
		...
		onDelete: function (e) {
			...
			var xhttp = new XMLHttpRequest();
			xhttp.open("DELETE", "/fethab/cfop/delete", true);
			xhttp.setRequestHeader('content-type', 'application/json');
